...
export class SidenavService implements OnDestroy {
  ...
  addItems(items: SidenavItem[]) {
    ...
    for (const item of items) {
      const foundIndex = itemsTmp.findIndex((existingItem) => isEqual(existingItem, item));
      if (foundIndex === -1) {
  ...
  reloadResources() {
   // this.service.requestUserResourceList()
    this.apiClient.getRequest('/api/usuario/user-resource-list')
      .subscribe({next: this.successMenu, error: console.error});
  }

  private successMenu = (body: any) => {
    this.addItems(body);
  }
  ...
  private getItemByRouteRecursive(route: string, collection: any[]) {
